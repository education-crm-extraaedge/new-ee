<?php
/**
 * Single Solution Template — mirrors single-industry.php exactly, plus the
 * solution-only Addon Features and Comprehensive Platform sections.
 *
 ...
 *  2. industry_seo_meta_tags() injects on wp_head:
...
$comp_h2        = get_post_meta($pid, '_comp_h2',        true);
$comp_sub       = get_post_meta($pid, '_comp_sub',       true);
$comp_items     = get_post_meta($pid, '_comp_items',     true) ?: array();
...
/* deployment marker */ echo "\n<!-- ee-solution-tpl v2026-07-23-fabs-sitewide -->\n";
?>
...
/* consent checkbox row — box + "Your contact details are used..." text aligned as one tidy flex row */
.hero-form-card input[type="checkbox"],#ee-form-7 input[type="checkbox"]{display:inline-block!important;float:none!important;width:15px!important;height:15px!important;min-width:15px!important;max-width:15px!important;flex:0 0 auto!important;margin:2px 8px 0 0!important;padding:0!important;accent-color:#DE6E30!important;vertical-align:top!important;box-shadow:none!important}
.hero-form-card label:has(input[type="checkbox"]),.hero-form-card div:has(>input[type="checkbox"]),.hero-form-card p:has(>input[type="checkbox"]),#ee-form-7 label:has(input[type="checkbox"]),#ee-form-7 div:has(>input[type="checkbox"]),#ee-form-7 p:has(>input[type="checkbox"]){display:flex!important;flex-direction:row!important;align-items:flex-start!important;gap:8px!important;width:100%!important;max-width:100%!important;margin:2px 0 12px!important;font-size:12px!important;font-weight:500!important;line-height:1.55!important;color:#5B6B84!important;text-align:left!important;text-transform:none!important;letter-spacing:0!important}
.hero-form-card label:has(input[type="checkbox"]) span,.hero-form-card label:has(input[type="checkbox"]) p,.hero-form-card div:has(>input[type="checkbox"]) label,.hero-form-card div:has(>input[type="checkbox"]) span,.hero-form-card div:has(>input[type="checkbox"]) p,#ee-form-7 label:has(input[type="checkbox"]) span,#ee-form-7 label:has(input[type="checkbox"]) p,#ee-form-7 div:has(>input[type="checkbox"]) label,#ee-form-7 div:has(>input[type="checkbox"]) span,#ee-form-7 div:has(>input[type="checkbox"]) p{display:inline!important;float:none!important;width:auto!important;max-width:100%!important;margin:0!important;font-size:12px!important;font-weight:500!important;line-height:1.55!important;color:#5B6B84!important;text-align:left!important}
.hero-form-card label:has(input[type="checkbox"]) a,.hero-form-card div:has(>input[type="checkbox"]) a,#ee-form-7 label:has(input[type="checkbox"]) a,#ee-form-7 div:has(>input[type="checkbox"]) a{color:#DE6E30!important;text-decoration:underline!important;font-weight:600!important}
/* fine print under the form ("Your contact details are used...") — clean left alignment */
.hero-form-card form p:not(:has(input)),#ee-form-7 form p:not(:has(input)){text-align:left!important;font-size:12px!important;font-weight:500!important;line-height:1.55!important;color:#5B6B84!important;margin:8px 0 12px!important;letter-spacing:0!important;text-transform:none!important}
...
/* Legacy logo-section / marquee-wrap CSS removed — the global
   ee_render_logo_marquee() owns the logo strip and brings its own
   styles; keeping these here re-introduced the side-fade overlay. */
...
.data-log{overflow:hidden;min-width:0;margin-top:20px;background:#111827;border-radius:10px;padding:12px;font-family:'Courier New',monospace;font-size:11px;color:#10B981;height:70px;overflow:hidden}
.log-line{margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.features-grid{display:flex;gap:10px;justify-content:center;align-items:stretch;flex-wrap:wrap;padding:0 24px}
.feat-card{flex:1 1 130px;max-width:160px;background:var(--white);border:1px solid rgba(25,51,93,.08);border-radius:16px;padding:18px 14px;display:flex;flex-direction:column;align-items:center;text-align:center;transition:var(--transition)}
...
.feat-img-wrap{width:100%;height:64px;background:transparent;border:0;margin-bottom:8px;display:flex;align-items:center;justify-content:center}
.feat-img{max-width:90%;max-height:90%;object-fit:contain}
.feat-title{font-family:var(--font-h);font-weight:700;font-size:11px;color:var(--blue);display:flex;flex-direction:column;align-items:center;gap:8px;transition:.3s}
...
/* ── Solution-only: addon features + comprehensive platform grids ── */
.addon-section{padding:40px 0;background:#fff}
.addon-head,.comp-head{text-align:center;margin-bottom:24px}
.addon-head h2,.comp-head h2{font-family:var(--font-h);font-weight:800;font-size:clamp(17px,2.2vw,22px);color:var(--blue)}
.addon-head p,.comp-head p{color:var(--gray-600);font-size:14px;max-width:640px;margin:8px auto 0}
.addon-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}
.addon-card{background:#fff;border:1px solid var(--gray-200);border-radius:16px;padding:22px;transition:var(--transition)}
.addon-card:hover{transform:translateY(-6px);box-shadow:var(--shadow-md);border-color:var(--orange)}
.addon-ic{width:46px;height:46px;border-radius:14px;background:rgba(222,110,48,.08);border:1px solid rgba(222,110,48,.18);display:flex;align-items:center;justify-content:center;font-size:22px;margin-bottom:12px}
.addon-card h3{font-weight:800;font-size:15px;color:var(--blue);margin-bottom:6px}
.addon-card p{font-size:13px;color:var(--gray-600);line-height:1.6}
.comp-section{padding:40px 0;background:var(--off-white)}
.comp-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}
.comp-card{background:#fff;border:1px solid var(--gray-200);border-radius:16px;padding:22px 18px;display:flex;flex-direction:column;transition:var(--transition)}
.comp-card:hover{transform:translateY(-6px);box-shadow:var(--shadow-md);border-color:var(--orange)}
.comp-ic{width:48px;height:48px;border-radius:14px;background:var(--blue);color:#fff;display:flex;align-items:center;justify-content:center;font-size:22px;margin-bottom:12px}
.comp-ic img{width:26px;height:26px;object-fit:contain}
.comp-card h3{font-weight:800;font-size:14.5px;color:var(--blue);margin-bottom:6px}
.comp-card p{font-size:12.5px;color:var(--gray-600);line-height:1.55;flex:1;margin-bottom:10px}
.comp-link{font-size:12px;font-weight:700;color:var(--orange);display:inline-flex;align-items:center;gap:5px;margin-top:auto}
.comp-link svg{width:12px;height:12px}
@media(max-width:1024px){.addon-grid,.comp-grid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:520px){.addon-grid,.comp-grid{grid-template-columns:repeat(2,1fr);gap:10px}.addon-card,.comp-card{padding:14px}}
      ...
      <article>
        ...
        <footer class="trust-bar reveal">
          ...
          <?php if(!empty($compliance)): ?><div class="compliance-row"><?php foreach($compliance as $comp):
            ...
            $ctext = trim((string)($comp['text'] ?? ($comp['label'] ?? '')));
            $cimg  = trim((string)($comp['image'] ?? ($comp['icon'] ?? '')));
    ...
    <nav class="toc-wrapper" id="toc" aria-label="Table of Contents">
      ...
      <ol class="toc-list" id="toc-list">
        ...
        if ($toc_enabled === 'custom' && !empty($toc_items)) {
            ...
            if (!empty($addon_features)) echo '<li class="toc-item"><a href="#additional-features" class="toc-link"><span class="toc-num">' . $n++ . '</span>' . esc_html($addon_h2 ?: 'More Features') . '</a></li>';
            if (!empty($comp_items))     echo '<li class="toc-item"><a href="#comprehensive" class="toc-link"><span class="toc-num">' . $n++ . '</span>' . esc_html($comp_h2 ?: 'Platform') . '</a></li>';
  ...
  <div class="toc-content-column">
    ...
    <section class="addon-section" id="additional-features" aria-labelledby="addon-h">
      <div class="container">
        ...
        <div class="addon-head reveal">
          <?php if($addon_h2): ?><h2 id="addon-h"><?php echo esc_html($addon_h2); ?></h2><?php endif; ?>
          <?php if($addon_sub): ?><p><?php echo ee_inline_links($addon_sub); ?></p><?php endif; ?>
        </div>
        <?php endif; ?>
    ...
    <section class="comp-section" id="comprehensive" aria-labelledby="comp-h">
      <div class="container">
        ...
        <div class="comp-head reveal">
          <?php if($comp_h2): ?><h2 id="comp-h"><?php echo esc_html($comp_h2); ?></h2><?php endif; ?>
          <?php if($comp_sub): ?><p><?php echo ee_inline_links($comp_sub); ?></p><?php endif; ?>
        </div>
        <?php endif; ?>
        <div class="comp-grid reveal">
          ...
          <div class="comp-card">
            ...
            <?php elseif(!empty($c['logo'])): ?><div class="comp-ic"><img src="<?php echo esc_url($c['logo']); ?>" alt="<?php echo esc_attr($c['title']); ?>"></div><?php endif; ?>
            <h3><?php echo esc_html($c['title']); ?></h3>
            <?php if(!empty($c['desc'])): ?><p><?php echo esc_html($c['desc']); ?></p><?php endif; ?>
            <a href="<?php echo esc_url(!empty($c['url']) ? $c['url'] : '#admission-form'); ?>" class="comp-link">Learn more<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
          </div>
          <?php endforeach; ?>
        </div>
      </div>
