<?php
/**
 * Single Solution Template — W3C VALID + SEO + AEO/GEO + CRAWLABILITY OPTIMIZED
 *
 * Clone of single-product.php — same admin editor (tabbed UI registered
 * in functions.php for 'product'/'industry'/'use_case'/'solution'), same
 * field set, same SEO + schema pipeline, same TOC layout. Editing a
 * Solution post in WP Admin shows the exact same screen as editing a
 * Product.
 *
 * ─── SOURCE ORDER (serial for Google + AI crawlers) ───
 *  1. Meta box data fetched once (PHP top)
 *  2. product_seo_meta_tags() injects on wp_head:
...
function solution_seo_meta_tags() {
...
add_action("wp_head", "solution_seo_meta_tags");
...
/* intl-tel-input phone field — make the flag dropdown sit cleanly on
   the left and push the input text 70px right so the placeholder /
