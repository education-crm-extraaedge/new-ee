...
/* SVG poster (gradient + play button) — matches the JS posterFor() */
...
.ee-cu{
  ...
  --line:rgba(25,51,93,.14);--line-soft:rgba(25,51,93,.07);
  ...
  background:var(--paper);color:var(--ink);font-family:'Hanken Grotesk',-apple-system,BlinkMacSystemFont,sans-serif;line-height:1.55;-webkit-font-smoothing:antialiased;position:relative;
}
...
.ee-cu .wrap{max-width:1180px;margin:0 auto;padding:56px 28px 0;position:relative;z-index:2}
...
.ee-cu .banner{position:relative;width:100%;aspect-ratio:16/9;overflow:hidden;flex-shrink:0;background:var(--card-grad,linear-gradient(135deg,var(--orange),var(--navy)));cursor:pointer}
...
.ee-cu .banner.has-thumb{background-size:cover;background-position:center}
...
.ee-cu .banner.playing .scrim,.ee-cu .banner.playing .b-tag,.ee-cu .banner.playing .b-metric,.ee-cu .banner.playing .play-btn{display:none}
...
.ee-cu .play-btn{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);z-index:3;width:62px;height:62px;border-radius:50%;background:rgba(252,249,243,.94);box-shadow:0 10px 30px rgba(0,0,0,.3);display:flex;align-items:center;justify-content:center;border:0;cursor:pointer;transition:transform .25s}
.ee-cu .banner:hover .play-btn{transform:translate(-50%,-50%) scale(1.08)}
.ee-cu .play-btn::after{content:"";border-style:solid;border-width:11px 0 11px 18px;border-color:transparent transparent transparent var(--navy);margin-left:5px}
...
.ee-cu ul.hl{list-style:none;margin-bottom:20px;display:flex;flex-direction:column;gap:9px;padding:0}
...
.ee-cu .cta{margin:96px 0 80px;background:var(--navy);border-radius:28px;padding:clamp(46px,7vw,80px);text-align:center;position:relative;overflow:hidden;color:#fff}
...
<main class="wrap" id="ee-stories">
  ...
  <div class="sec-head">
    ...
    <?php if (!empty($cu_set['sec_sub'])): ?><p><?php echo esc_html($cu_set['sec_sub']); ?></p><?php endif; ?>
  </div>
  ...
  <div class="grid" role="list" aria-label="Customer success stories">
    <?php foreach ($cu_stories as $i => $s):
      ...
      list($vtype, $vembed, $vyt_thumb) = $ee_classify_video($vid);
      $banner_thumb = $thumb ?: $vyt_thumb;
      $banner_class = 'banner' . ($banner_thumb ? ' has-thumb' : '');
      $banner_style = "--card-grad:$grad" . ($banner_thumb ? ";background-image:url('" . esc_url($banner_thumb) . "')" : '');
    ?>
      <article class="card" data-cat="<?php echo esc_attr($cat); ?>">
        <div class="<?php echo esc_attr($banner_class); ?>" style="<?php echo esc_attr($banner_style); ?>"
          <?php if ($vtype): ?>data-vtype="<?php echo esc_attr($vtype); ?>" data-vsrc="<?php echo esc_attr($vembed); ?>" data-vtitle="<?php echo esc_attr($title); ?>"<?php endif; ?>>
          <?php if (!$banner_thumb && !$vtype): ?>
          ...
          <?php if ($vtype): ?><button type="button" class="play-btn" aria-label="Play video"></button><?php endif; ?>
        </div>
        <div class="body">
          <div class="toprow"><span class="cat <?php echo esc_attr($cat); ?>"><?php echo esc_html($cat); ?></span></div>
...
(function(){
  ...
  /* In-card video swap on banner click */
  function stopOthers(except){
    root.querySelectorAll('.banner.playing').forEach(function(b){
      if (b === except) return;
      var el = b.querySelector('iframe,video');
      if (el) el.parentNode.removeChild(el);
      b.classList.remove('playing');
    });
  }
  root.querySelectorAll('.banner[data-vsrc]').forEach(function(banner){
    banner.addEventListener('click', function(){
      if (banner.classList.contains('playing')) return;
      stopOthers(banner);
      var type = banner.dataset.vtype;
      var src  = banner.dataset.vsrc;
      var el;
      if (type === 'mp4'){
        ...
        el.src = src; el.title = banner.dataset.vtitle || '';
